...
Cobra provides command parsing, subcommand grouping, flag definitions, and auto-generated help text. We use Cobra command groups (`Inspect:`, `Diagnose:`, `Utility:`) to keep `--help` readable as the CLI grows.

**Why Cobra:** It is the de facto standard for Kubernetes-adjacent CLIs (`kubectl` plugins, `helm`, and many others). It integrates cleanly with `pflag` and supports the grouped help layout we wanted.

### Bubble Tea (`github.com/charmbracelet/bubbletea`)

Bubble Tea implements the **Model–Update–View** pattern for terminal applications. Each interactive surface (inspect viewer, diagnose results browser, dataset picker, LLM config form) is a separate Bubble Tea program.

**Why Bubble Tea:** Go-native TUI framework with explicit state management, keyboard handling, and alt-screen support. It keeps UI logic in Go without curses bindings.

### Bubbles (`github.com/charmbracelet/bubbles`)

Bubbles provides reusable TUI components:

- **`table`** — scrollable artifact and resource tables in inspect/diagnose views.
- **`list`** — fuzzy-filterable dataset picker when `fluid inspect` is run without a dataset name.
- **`textinput`** — form fields in `fluid diagnose config`.

### Lip Gloss (`github.com/charmbracelet/lipgloss`)

Lip Gloss handles terminal styling: titles, tabs, panels, and subtle help text. Shared styles live in `pkg/tui/common/styles.go` so inspect and diagnose views feel consistent.

### Kubernetes client libraries

| Library | Role |
|---------|------|
| `k8s.io/cli-runtime/pkg/genericclioptions` | Kubeconfig and context flags |
| `sigs.k8s.io/controller-runtime/pkg/client` | Typed access to Fluid CRDs |
| `k8s.io/client-go/kubernetes` | Pod logs, events, and typed core API calls |
| `github.com/fluid-cloudnative/fluid/api/v1alpha1` | Fluid Dataset and Runtime types |

We register Fluid and core types in `pkg/scheme/scheme.go` so a single controller-runtime client can list Datasets, Runtimes, and related objects.

### `golang.org/x/term`

Used in `pkg/tui/common/interactive.go` to detect whether stdin/stdout are real terminals. TUI modes refuse to start in piped or CI environments and return a clear error suggesting non-interactive output flags.

### What we did not use

- **Viper** — configuration is a small YAML file with explicit load/save logic in `pkg/diagnose/config.go`.
- **A plugin framework** — not part of the current design.
