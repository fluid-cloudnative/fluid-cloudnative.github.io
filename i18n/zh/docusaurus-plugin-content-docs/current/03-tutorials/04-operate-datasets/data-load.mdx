...
默认情况下，上述DataLoad配置将会尝试预热Dataset中的全部数据，如果你希望仅加载Dataset中的指定子路径（我们也强烈推荐这么做），请参考[DataLoad进阶配置](#指定一个或多个子目录进行加载)
...
spec:
  ...
  # highlight-start
  policy: Cron
  schedule: "0 0 * * 1-5" # Load data at each 00:00 from Monday to Friday
  # highlight-end
  target:
    - path: /
```
